...
const useCucumberTsFlow = () => {
  ...
  const checkCucumberTsFlow = (): boolean => {
    if (
      isCucumberTsFlowInstalled() &&
      semverGte(tsFlowVersion, minCucumberTsFlowVer)
    ) {
      return true;
    }
    if (isCucumberTsFlowInstalled()) {
      console.log(
        `Current version of cucumber-tsflow is: \"${tsFlowVersion}\"`
      );
    }
    console.log(
      `Cucmber TsFlow for VSCode requires cucumber-tsflow version \"${minCucumberTsFlowVer}\" or higher.`
    );
    return false;
  };
