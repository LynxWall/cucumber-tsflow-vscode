...
export class CtvConfig {
   ...
   */
  public get configFilePath(): string | undefined {
    return vscode.workspace
      .getConfiguration()
      .get("cucumber-tsflow.configFilePath");
  }
  ...
  public get profile(): string {
    return (
      vscode.workspace.getConfiguration().get("cucumber-tsflow.profile") ??
      "default"
    );
  }
  ...
  public get cucumberPath(): string | undefined {
    return (
      vscode.workspace.getConfiguration().get("cucumber-tsflow.cucumberPath") ||
      this.currentCucumberRootPath
    );
  }
  ...
  public get runOptions(): string[] | null {
    const runOptions = vscode.workspace
      .getConfiguration()
      .get("cucumber-tsflow.runOptions");
    if (runOptions) {
  ...
  public get debugOptions(): Partial<vscode.DebugConfiguration> {
    const debugOptions = vscode.workspace
      .getConfiguration()
      .get("cucumber-tsflow.debugOptions");
    if (debugOptions) {
  ...
  public get runCommand(): string {
    const runCommand: string | undefined = vscode.workspace
      .getConfiguration()
      .get("cucumber-tsflow.runCommand");
    if (runCommand) {
      return runCommand;
    }
  ...
  /**
   * Disable CodeLens feature
   */
  public get disableCodeLens(): boolean {
    return (
      vscode.workspace
        .getConfiguration()
        .get("cucumber-tsflow.disableCodeLens") || false
    );
  }

  /**
   * CodeLens will be shown on files matching this pattern
   */
  public get stepFileSelector(): string {
    const glob =
      vscode.workspace
        .getConfiguration()
        .get("cucumber-tsflow.stepFileSelector") || "";
   ...
   * CodeLens will be shown on files matching this pattern
   */
  public get featureFileSelector(): string {
    const glob =
      vscode.workspace
        .getConfiguration()
        .get("cucumber-tsflow.featureFileSelector") || "";
    return normalizePath(glob as string);
  }
  ...
  public get preserveEditorFocus(): boolean {
    return (
      vscode.workspace
        .getConfiguration()
        .get("cucumber-tsflow.preserveEditorFocus") || false
    );
  }
  ...
  public get cucumberBinPath(): string {
    // default
    const fallbackRelativeTsFlowBinPath =
    ...
    let tsflowPath = relativeTsFlowBin.find((relativeBin) =>
      isNodeExecuteAbleFile(path.join(cwd, relativeBin))
    );
  ...
  private get currentWorkspaceRootPath(): string | undefined {
    ...
    if (editor) {
      let currentFolderPath: string | undefined = path.dirname(
  ...
  private get currentWorkspaceFolderPath(): string | undefined {
    const editor = vscode.window.activeTextEditor;
    if (editor) {
      return vscode.workspace.getWorkspaceFolder(editor.document.uri)?.uri
        .fsPath;
    }
    return undefined;
  }
