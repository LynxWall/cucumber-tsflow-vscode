...
import {
  ...
  Dialect,
  dialects,
} from "@cucumber/gherkin/dist/src";
import {
  ParsedFeature,
  ParsedScenario,
  ParsedStep,
...
export default class GherkinFeature {
  ...
  public loadFeatures = (globPattern: string, options?: Options) => {
    ...
    return featureFiles.map((featureFilePath) =>
      this.loadFeature(featureFilePath, options)
    );
  };
  ...
  private parseScenarioOutlineExample = (
    ...
    return {
      title: this.getOutlineDynamicTitle(
        exampleTableRow,
        outlineScenario.title
      ),
      steps: this.parseScenarioOutlineExampleSteps(
        exampleTableRow,
  ...
  private parseScenarioOutlineExampleSet = (
    ...
    const exampleTable = this.parseDataTable(
      astExampleSet.tableBody,
      astExampleSet.tableHeader
    );

    return exampleTable.map((tableRow) =>
      this.parseScenarioOutlineExample(
        tableRow,
        outlineScenario,
        this.parseTags(astExampleSet)
      )
    );
  };
  ...
  private parseScenarioOutlineExampleSets = (
    ...
    const exampleSets = astExampleSets.map((astExampleSet: any) => {
      return this.parseScenarioOutlineExampleSet(
        astExampleSet,
        outlineScenario
      );
    });
  ...
  private parseScenarioOutline = (astScenarioOutline: any, astFeature: any) => {
    const outlineScenario = this.parseScenario(
      astScenarioOutline.scenario,
      astFeature
    );

    return {
      title: outlineScenario.title,
      scenarios: this.parseScenarioOutlineExampleSets(
  ...
  private parseScenarios = (astFeature: any) => {
    return astFeature.children
      ...
      .map((astScenario: any) =>
        this.parseScenario(astScenario.scenario, astFeature)
      );
  };
  ...
  private parseScenarioOutlines = (astFeature: any) => {
    return astFeature.children
      ...
      .map((astScenarioOutline: any) =>
        this.parseScenarioOutline(astScenarioOutline, astFeature)
      );
  };
  ...
  private collapseRulesAndBackgrounds = (astFeature: any) => {
    ...
    const children = this.collapseBackgrounds(
      astFeature.children,
      featureBackgrounds
      ...
      if (nextChild.rule) {
        ...
        return [
          ...newChildren,
          ...this.collapseBackgrounds(rule.children, [
            ...featureBackgrounds,
            ...ruleBackgrounds,
          ]),
        ];
