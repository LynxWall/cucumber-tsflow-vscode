...
import GherkinManager, {
  StepInfo,
  IMapFeaturesResult,
} from "../gherkin/gherkin-manager";
...
class StepFileManager {
   ...
   * @returns
   */
  public getFeatureAndScenario = (stepText: string) => {
    if (this.findFeatureResults?.features) {
      return this.findFeatureResults?.features.getMatchingFeatureAndScenario(
        stepText
      );
    }
  };
