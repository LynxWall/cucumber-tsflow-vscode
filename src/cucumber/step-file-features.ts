import { ParsedFeature, ParsedScenario, ParsedStep } from "../gherkin/models";
import {
  FeatureFromStepFile,
...
const stepFileFeatures = () => {
  ...
  return { upsertFeature, getMatchingFeatureAndScenario };
};
...
const useStepFileScenario = (scenario: ScenarioFromStepFile) => {
  const upsertStep = (step: ParsedStep) => {
    let sfStep = scenario.steps.find(
      (s) => s.keyword === step.keyword && s.stepText === step.stepText
    );
    if (!sfStep) {
      sfStep = {
  ...
  const hasStepText = (stepText: string): boolean => {
    const match =
      scenario.steps.find((s) =>
        hasMatchingStep(stepText.toLowerCase(), s.stepText.toLowerCase())
      ) !== undefined;
    return match;
  };
